...
<body>
	...
	<?php if(isset($_POST["addSaleRecord"])){
		//Get Data From Form
		$drugId=(int) $_POST["drugName"];	$quantity=(int) $_POST["quantity"];
		$date=$_POST["dateSold"]; $usage=$_POST["usage"];
		$soldBy=$_SESSION["regNo"];
		//Get Quantity From Stock
		$sql1=mysqli_query($connection,"SELECT id, quantity, price, name from stock WHERE id={$drugId}");
		
		$getQuantity=mysqli_fetch_array($sql1);
		$newQuantity=$getQuantity["quantity"]-$quantity;
		$price=$quantity * $getQuantity["price"];
		$drugName=$getQuantity["name"];
		//Update Quantity
		...
		."price='{$price}', dateSold='{$date}', drugUsage='{$usage}', soldBy='{$soldBy}'";
	...
	<h3 class="text-bold text-center">PHARMACY MANAGEMENT SYSTEM</h3>
